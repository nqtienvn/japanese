...
@Slf4j
@Component
public class JWTClientCookieFilter extends OncePerRequestFilter {
	private final JWTTokenProvider<?> jwtTokenProvider;

	private JwtParser jwtParser;

	public JWTClientCookieFilter(JWTTokenProvider<?> jwtTokenProvider) {
		...
		if (cookies != null) {
			...
			if (accessCookie != null) {
				...
				if (StringUtils.hasText(accessToken) && StringUtils.hasText(csrfToken)
						&& this.jwtTokenProvider.validateTokenPublicKey(accessToken)
						&& this.jwtTokenProvider.validateTokenPublicKey(csrfToken)) {
