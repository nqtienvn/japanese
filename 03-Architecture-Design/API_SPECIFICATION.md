...
- [ ] Every API/event ID is linked to a requirement, use case, and test case.
- [ ] Request/input data is clearly separated, with specific boundary/invalid/empty behavior defined.
- [ ] Success/error status and observable state/side effects are defined, avoiding vague "error report" terms.
- [ ] Permissions, idempotency, concurrency, timeout/retry/fallback, and compatibility rules are clear.
- [ ] Machine-readable contracts are validated automatically where stack support is available.
