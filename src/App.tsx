...
function Flashcards({ terms, onRate }: { terms: Term[]; onRate: (outcome: { termId: string; mode: Mode; correct: boolean | null; rating?: 'remembered' | 'needs_review' }) => Promise<void> }) {
  const [source, setSource] = useState<'all' | 'flashcard'>('flashcard'); const [count, setCount] = useState('10'); const [deck, setDeck] = useState<Term[] | null>(null); const [index, setIndex] = useState(0); const [flipped, setFlipped] = useState(false); const [saving, setSaving] = useState(false);
  const pool = source === 'all' ? terms : terms.filter((term) => term.modes.includes('flashcard'));
  const term = deck?.[index];
  const begin = () => { setDeck(shuffle(pool).slice(0, countFor(count, pool.length))); setIndex(0); setFlipped(false); };
  if (!deck) return <section className="session"><h2>Flashcard</h2><p>Lật thẻ để xem mặt còn lại rồi tự đánh giá mức nhớ.</p><div className="config-grid"><label>Nguồn từ<select value={source} onChange={(event) => setSource(event.target.value as 'all' | 'flashcard')}><option value="flashcard">Chỉ từ đánh dấu Flashcard</option><option value="all">Tất cả từ đang dùng</option></select></label><label>Số thẻ<select value={count} onChange={(event) => setCount(event.target.value)}><option value="10">10 thẻ</option><option value="20">20 thẻ</option><option value="all">Tất cả ({pool.length})</option></select></label></div><button className="primary" disabled={!pool.length} onClick={begin}>Bắt đầu Flashcard</button></section>;
  if (!term) return <section className="session"><h2>Hoàn thành Flashcard</h2><p>Bạn đã tự đánh giá {deck.length} thẻ.</p><button className="primary" onClick={() => setDeck(null)}>Tạo lượt mới</button></section>;
  ...
  return <section className="session"><p className="eyebrow">FLASHCARD · {index + 1}/{deck.length}</p><button type="button" className="card" onClick={() => setFlipped(!flipped)} onKeyDown={(event) => { if (event.key === ' ' || event.key === 'Enter') { event.preventDefault(); setFlipped(!flipped); } }}><span>{flipped ? term.vietnamese : term.japanese}</span><small>{flipped ? 'Nhấn để xem tiếng Nhật' : 'Nhấn Space/Enter để lật'}</small></button>{flipped && <div className="actions"><button className="primary" disabled={saving} onClick={() => void rate(true)}>Đã nhớ</button><button disabled={saving} onClick={() => void rate(false)}>Cần ôn</button></div>}</section>;
}
...
function Quiz({ terms, onQuizComplete }: { terms: Term[]; onQuizComplete: (answers: QuizAnswer[]) => Promise<void> }) {
  ...
  async function finishQuiz() {
    ...
    setBusy(true); setError('');
    try {
      try { await saveCurrent(); } catch (saveError) { if (Date.now() < new Date(attempt.deadlineAt).getTime()) throw saveError; }
      const nextResult = await api.submitQuiz(attempt.id); const finalAnswers = await api.loadQuizAnswers(attempt.id); setAnswers(finalAnswers); setResult(nextResult); await onQuizComplete(finalAnswers);
    } catch (nextError) { setError(messageOf(nextError)); } finally { setBusy(false); }
  }
